	...
	def create(self):
		...
		with _sm_container_0:
			...
			OperatableStateMachine.add('ask destination',
										SaraSay(sentence="Ok, where do you want me to go", emotion=1, block=True),
			...
			OperatableStateMachine.add('sorry',
										SaraSay(sentence="Sorry, I think I'm stuck", emotion=1, block=True),
			...
			OperatableStateMachine.add('say',
										SaraSay(sentence="Sorry, I don't know this place.", emotion=1, block=True),
										transitions={'done': 'done'},
										autonomy={'done': Autonomy.Off})

			# x:411 y:393
			OperatableStateMachine.add('say kitchen',
										SaraSay(sentence="Okay, I'm going to the kitchen", emotion=1, block=True),
										transitions={'done': 'gen kitchen'},
										autonomy={'done': Autonomy.Off})

			# x:423 y:521
			OperatableStateMachine.add('say room',
										SaraSay(sentence="Okay, I'm going to my room", emotion=1, block=True),
										transitions={'done': 'gen room'},
										autonomy={'done': Autonomy.Off})

			# x:422 y:270
			OperatableStateMachine.add('say door',
										SaraSay(sentence="Okay, I'm coming back to the door.", emotion=1, block=True),
		...
		with _sm_repeat_group_1:
			...
			OperatableStateMachine.add('say1',
										SaraSay(sentence="Say anything and I'll repeat after you.", emotion=1, block=True),
		...
		with _sm_show_gripper_2:
			...
			OperatableStateMachine.add('respond gripper',
										SaraSay(sentence="My gripper is build by Robotiq and allows me to pick up things.", emotion=1, block=True),
			...
			OperatableStateMachine.add('close',
										...
										transitions={'object': 'say catch', 'no_object': 'say catch'},
			...
			OperatableStateMachine.add('open',
										...
										transitions={'object': 'lower', 'no_object': 'lower'},
										autonomy={'object': Autonomy.Off, 'no_object': Autonomy.Off},
										remapping={'object_size': 'object_size'})

			# x:63 y:389
			OperatableStateMachine.add('say catch',
										SaraSay(sentence="I am an excellent catch", emotion=1, block=True),
			...
			OperatableStateMachine.add('opp',
										SetGripperState(width=0.1, effort=0),
										transitions={'object': 'show', 'no_object': 'show'},
										autonomy={'object': Autonomy.Off, 'no_object': Autonomy.Off},
										remapping={'object_size': 'object_size'})
		...
		with _sm_show_base_3:
			...
			OperatableStateMachine.add('respond base',
										SaraSay(sentence="My base is what allows me to move. I can go in any direction thanks to my mecanum wheels and motor controllers.", emotion=1, block=True),
										transitions={'done': 'done'},
										autonomy={'done': Autonomy.Off})
		...
		with _sm_show_brain_4:
			...
			OperatableStateMachine.add('say brain',
										SaraSay(sentence="My brain is located inside my laptop. You can call me a no brainer.", emotion=1, block=True),
										transitions={'done': 'done'},
										autonomy={'done': Autonomy.Off})
		...
		with _sm_show_arm_5:
			...
			OperatableStateMachine.add('say arm',
										SaraSay(sentence="My arm is custom made from 3d printed titanium and use Kinova high precision harmonic drives to move. Watch my moves!", emotion=1, block=True),
			...
			OperatableStateMachine.add('say cap',
										SaraSay(sentence="With these, I can lift up to 2 pounds.", emotion=1, block=True),
			...
			OperatableStateMachine.add('sayerror',
										SaraSay(sentence="well, It does that from time to time. Anyway let's move on.", emotion=1, block=True),
		...
		with _sm_show_head_6:
			...
			OperatableStateMachine.add('say head',
										SaraSay(sentence="My head is composed of my eyes, which are made of a 3d camera. I also have 80 LED that allows me to simulate human emotions.", emotion=1, block=True),
			...
			OperatableStateMachine.add('say hard',
										SaraSay(sentence="Human emotion are so complicated.", emotion=1, block=True),
		...
		with _sm_get_name_7:
			...
			OperatableStateMachine.add('ask for name',
										SaraSay(sentence="Just say your name after the beep sound", emotion=1, block=True),
										transitions={'done': 'wait1'},
										autonomy={'done': Autonomy.Off})

			# x:41 y:343
			OperatableStateMachine.add('say name',
										SaraSayKey(Format=lambda x: "Did you say your name was "+x+"?", emotion=1, block=True),
			...
			# x:232 y:260
			OperatableStateMachine.add('sorry',
										SaraSay(sentence="Sorry, I'm not good with names. Just say your name again after the beep sound.", emotion=1, block=True),
										transitions={'done': 'happy'},
										autonomy={'done': Autonomy.Off})
		...
		with _sm_questions_8:
			...
			OperatableStateMachine.add('say question',
										SaraSayKey(Format=lambda x: "Now "+x+". You can ask me about my head, arm, hand, base, wheel and brain.", emotion=1, block=True),
			...
			# x:382 y:88
			OperatableStateMachine.add('Sorry',
										SaraSay(sentence="sorry I did not understand", emotion=1, block=True),
										transitions={'done': 'ask for questions'},
										autonomy={'done': Autonomy.Off})
			...
			OperatableStateMachine.add('ask for questions',
										SaraSay(sentence="Just ask your question after the beep sound", emotion=1, block=True),
										transitions={'done': 'happy'},
										autonomy={'done': Autonomy.Off})

			# x:217 y:351
			OperatableStateMachine.add('call other say',
										SaraSay(sentence="You can ask me another question if you want", emotion=1, block=True),
										transitions={'done': 'ask for questions'},
										autonomy={'done': Autonomy.Off})
		...
		with _sm_greet_9:
			...
			OperatableStateMachine.add('say hello',
										SaraSay(sentence="Hi, my name is sarah. I am a human assistant robot made by the student club Walking Machine of ETS.", emotion=1, block=True),
			...
			OperatableStateMachine.add('say greet',
										SaraSayKey(Format=lambda x: "Nice to meet you "+x+". I am happy to see you today. We never get visitors around here.", emotion=1, block=True),
										transitions={'done': 'wait4'},
										autonomy={'done': Autonomy.Off},
										remapping={'sentence': 'name'})
			...
			OperatableStateMachine.add('call name',
										SaraSay(sentence="Could you tell me your name please?", emotion=1, block=True),
		...
		with _state_machine:
			...
			OperatableStateMachine.add('goodbye',
										SaraSay(sentence="goodbye", emotion=1, block=True),
										transitions={'done': 'Get_speech'},
										autonomy={'done': Autonomy.Off})

			# x:324 y:541
			OperatableStateMachine.add('say',
										SaraSay(sentence="bye", emotion=1, block=True),
										transitions={'done': 'Get_speech'},
										autonomy={'done': Autonomy.Off})
			...
			OperatableStateMachine.add('sayend',
										SaraSay(sentence="Ok, that's enough. Please leave now.", emotion=1, block=True),
										transitions={'done': 'Get_speech_2'},
										autonomy={'done': Autonomy.Off})
