...
class ActionWrapper_BringSM(Behavior):
    ...
    def create(self):
        ...
        _sm_get_room_1 = OperatableStateMachine(outcomes=['found', 'unknown', 'error'], input_keys=['Action', 'robot_pose'], output_keys=['expected_pose'])
        ...
        with _sm_get_room_1:
            ...
            OperatableStateMachine.add('get room',
                                        WonderlandGetRoom(),
                                        transitions={'found': 'found', 'unknown': 'unknown', 'error': 'error'},
                                        autonomy={'found': Autonomy.Off, 'unknown': Autonomy.Off, 'error': Autonomy.Off},
                                        remapping={'id': 'id', 'name': 'name', 'type': 'type', 'expected_pose': 'robot_pose', 'room_pose': 'expected_pose', 'room_name': 'room_name', 'room_type': 'room_type'})


        # x:844 y:545, x:854 y:48
        _sm_bring_2 = OperatableStateMachine(outcomes=['finished', 'failed'], input_keys=['expected_pose', 'robot_pose', 'Action'])
        ...
        with _sm_bring_2:
            ...
            OperatableStateMachine.add('get object',
                                        WonderlandGetObject(),
                                        transitions={'found': 'Move_to_Object', 'unknown': 'finished', 'error': 'failed'},
                                        autonomy={'found': Autonomy.Off, 'unknown': Autonomy.Off, 'error': Autonomy.Off},
        ...
        with _state_machine:
            ...
            OperatableStateMachine.add('say3',
                                        SaraSayKey(Format=lambda x: "I'm now gonna bring the "+x[1]+" to "+x[2], emotion=1, block=True),
                                        transitions={'done': 'get beneficiary'},
                                        autonomy={'done': Autonomy.Off},
            ...
            OperatableStateMachine.add('say bring',
                                        ...
                                        transitions={'done': 'get object name'},
                                        autonomy={'done': Autonomy.Off},
                                        remapping={'sentence': 'Action'})
            ...
            # x:22 y:594
            OperatableStateMachine.add('get beneficiary',
                                        CalculationState(calculation=lambda x: x[3]),
                                        transitions={'done': 'get object name'},
                                        autonomy={'done': Autonomy.Off},
                                        remapping={'input_value': 'Action', 'output_value': 'beneficiary'})
            ...
            OperatableStateMachine.add('Bring',
                                        ...
                                        transitions={'finished': 'finished', 'failed': 'failed'},
                                        autonomy={'finished': Autonomy.Inherit, 'failed': Autonomy.Inherit},
                                        remapping={'expected_pose': 'expected_pose', 'robot_pose': 'robot_pose', 'Action': 'Action'})
            ...
            OperatableStateMachine.add('Get room',
                                        _sm_get_room_1,
                                        transitions={'found': 'cond3', 'unknown': 'get expected pose', 'error': 'failed'},
                                        autonomy={'found': Autonomy.Inherit, 'unknown': Autonomy.Inherit, 'error': Autonomy.Inherit},
                                        remapping={'Action': 'Action', 'robot_pose': 'robot_pose', 'expected_pose': 'expected_pose'})
