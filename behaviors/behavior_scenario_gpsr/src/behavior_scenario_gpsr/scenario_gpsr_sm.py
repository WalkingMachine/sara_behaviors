...
class Scenario_GPSRSM(Behavior):
	...
	def create(self):
		# x:461 y:37, x:466 y:140
		_state_machine = OperatableStateMachine(outcomes=['finished', 'failed'])
		_state_machine.userdata.ActionGoToStart = ["Move", "GPSR"]
		...
		with _sm_do_the_actions_1:
			...
			OperatableStateMachine.add('Action_Executor',
										...
										transitions={'finished': '++i', 'failed': 'failed', 'critical_fail': 'failed'},
		...
		with _state_machine:
			...
			OperatableStateMachine.add('for 3',
										ForLoop(repeat=3),
										transitions={'do': 'Action_Executor', 'end': 'finished'},
			...
			OperatableStateMachine.add('Do the actions',
										...
										transitions={'finished': 'for 3', 'failed': 'Fail state', 'critical fail': 'failed'},
			...
			OperatableStateMachine.add('Action_Executor',
										...
										transitions={'finished': 'GetOriginalPose', 'failed': 'failed', 'critical_fail': 'failed'},
										autonomy={'finished': Autonomy.Inherit, 'failed': Autonomy.Inherit, 'critical_fail': Autonomy.Inherit},
