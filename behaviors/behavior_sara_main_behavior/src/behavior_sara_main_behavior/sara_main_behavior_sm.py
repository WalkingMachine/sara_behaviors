	...
	def create(self):
		...
		with _sm_stopped_mode_0:
			...
			OperatableStateMachine.add('oups',
										SaraSay(sentence="Oh! You pushed my stop button", emotion=1, block=True),
		...
		# x:841 y:231
		_sm_sara_action_executor_2 = OperatableStateMachine(outcomes=['shutdown'], input_keys=['HighFIFO', 'MedFIFO', 'LowFIFO', 'DoNow', 'End'])
		...
		# x:887 y:420
		_sm_sara_brain_3 = OperatableStateMachine(outcomes=['error'], input_keys=['HighFIFO', 'LowFIFO', 'MedFIFO', 'DoNow', 'End'])
		...
		# x:55 y:366
		_sm_create_fifos_4 = OperatableStateMachine(outcomes=['done'], output_keys=['HighFIFO', 'MedFIFO', 'LowFIFO', 'DoNow'])

		with _sm_create_fifos_4:
			# x:33 y:40
		...
		# x:291 y:514, x:284 y:353, x:276 y:271, x:286 y:427
