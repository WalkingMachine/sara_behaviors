...
class GetPositionToPlaceOnTable(EventState):
    ...
    def __init__(self):
        ...
        self.planeTopic = "/plane"
        self.planeSub = ProxySubscriberCached({self.planeTopic: PointCloud})
    ...
    def execute(self, userdata):
        ...
        if self.plane is not None and mypose is not None:
            ...
            closest_point = self.plane.points[0]
            min_dist = 99999
            ...
            center = Point()
            center.x = sum_x / len(self.plane.points)
            center.y = sum_y / len(self.plane.points)
            center.z = sum_z / len(self.plane.points)
